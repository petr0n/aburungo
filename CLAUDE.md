...
- **Content ships with the book map.** The browsable map is the `bookmap/` SPA (`pnpm bookmap`):
  it renders the app's own content modules, so it can never go stale and a new book appears the
  moment `src/content/books.ts` carries it. The diff-able record is `docs/<book>-ladder.md`,
  generated by `pnpm ladder` — any change under `src/content/` must regenerate it in the same
  commit. `pnpm test` fails while it is stale, and a chapters file whose id prefix has no `BOOKS`
  entry in `scripts/ladder.mjs` makes generation throw, so no book content can land off the map.
