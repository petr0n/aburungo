...
- **Checkpoints run on a cadence.** Roughly every ten teaching units, placed at the next *situation boundary* — never mid-situation, since a sweep between "at the café" and "how much & paying" consolidates nothing. Treat ~10 as the target when authoring new content rather than a rule to retrofit against existing situations. Consolidation (a sweep, recurring) and completion (once, at the end) are different things; do not merge them. See DR-021.
