...
## 6. KRADFILE (kanji component decomposition)

**Use for:** the "shapes in this kanji" row on the kanji intro card — which visual pieces a
character is built from, so a new kanji can be introduced as parts the learner has already met.

**Immediate source:** https://github.com/hoffmannjp/krad-unicode (`krad.json`) — a Unicode
conversion of the EDRDG's KRADFILE, which is distributed in EUC-JP with its own line format.
**Upstream:** KRADFILE, (c) James William Breen and the Electronic Dictionary Research and
Development Group, https://www.edrdg.org/krad/kradinf.html
**Coverage:** 6,355 kanji decomposed into 253 visual components. Flat: kanji in, component list
out, no tree.

**Generated by:** `node scripts/kanji.mjs decompose` → `src/content/kanji/decomposition.json`,
scoped to the characters `src/content/lessons/*.yaml` actually teach.
...
**Not KANJIDIC2:** it carries one *classifying* radical per kanji, which is a different thing.
**Not KanjiVG:** its groupings are drawn for stroke order, not for teaching, and cost XML parsing.

---

## 7. VOICEVOX (Self-hosted TTS)
...
## 9. OpenAI Whisper API (STT upgrade path)
...
## 10. Claude Haiku (Conversation Sessions)
...
## 11. OpenAI Realtime API (Voice Conversation — Future Consideration)
...
| Kanji components | KRADFILE via krad-unicode `krad.json` | Generated table ships its CC BY-SA header; keywords stay in a separate file |
