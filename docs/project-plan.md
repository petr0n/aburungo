...
- **Book One** (N5 internally; a learner never reads that) — as of 2026-08-20: 87 lessons across
  10 chapters, 423 words, 191 phrases, 39 grammar patterns, 163 kanji, 19 situations.
  The live map is `docs/book-one-ladder.md`, generated by `pnpm ladder` and test-enforced; do not
  hand-write these counts anywhere else.
- All content tagged `jlpt: N5`, marked `content-source: training`, headwords verified against
  JMdict for Applications 3.6.2. Composed sentences remain pending Tatoeba verification.
...
- **Assessment language is non-judgmental.** A judged answer reads "Correct" or "Not quite" (changed 2026-08-21); avoid "missed" and "retest" -- prefer "review again". A score is a momentary reflection, never a grade. See assessment principles below.
...
- **Scores are reflection, not judgment.** Show counts ("7 correct") but never percentages, pass/fail verdicts, or persistent grades. The real feedback is what the SRS surfaces next — not a number.
