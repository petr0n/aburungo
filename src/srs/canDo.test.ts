...
function unit(id: string, order: number, situation: string, wordIds: string[], checkpoint?: Unit["checkpoint"]): Unit {
  return {
    id,
    order,
    situation,
...
describe("taughtSituations", () => {
  it("lists distinct situations in ladder order, once each", () => {
    expect(taughtSituations(units, ["u1", "u2", "u3"])).toEqual(["Greetings", "Food & drink"]);
  });

  it("ignores units the learner has not reached", () => {
    expect(taughtSituations(units, ["u1"])).toEqual(["Greetings"]);
  });
  ...
  it("never offers a checkpoint as a situation to role-play", () => {
    ...
    expect(taughtSituations(units, ["u1", "cp"])).toEqual(["Greetings"]);
  });
});

describe("wordsForSituations", () => {
  it("gathers every word across the situation's units", () => {
    expect(wordsForSituations(units, ["Greetings"], words).map((w) => w.id)).toEqual(["a", "b", "c"]);
  });

  it("deduplicates a word taught in two units", () => {
    const repeated = [...units, unit("u5", 6, "Greetings", ["a"])];
  ...
  it("skips ids with no matching word rather than emitting holes", () => {
    const dangling = [unit("x", 1, "Greetings", ["a", "missing"])];
...
describe("buildCrossSituationScope", () => {
  it("names both situations so Hana has a scene change to play", () => {
    const scope = buildCrossSituationScope(units, ["u1", "u2", "u3"], words, noShuffle);
  ...
  it("returns null when only one situation has been taught", () => {
    ...
    expect(buildCrossSituationScope(units, ["u1", "u2"], words, noShuffle)).toBeNull();
  });

  it("returns null on an empty ladder", () => {
    expect(buildCrossSituationScope(units, [], words, noShuffle)).toBeNull();
  });

  it("caps the word list, since prompt size is API spend", () => {
    const many = Array.from({ length: 90 }, (_, i) => word(`w${i}`, `theme-${i % 4}`));
    const big = [unit("b1", 1, "A", many.slice(0, 45).map((w) => w.id)), unit("b2", 2, "B", many.slice(45).map((w) => w.id))];
...
describe("buildCanDoScope", () => {
  it("scopes to the one situation the learner picked", () => {
    const scope = buildCanDoScope(units, "Food & drink", words);
  ...
  it("returns null for a situation with no vocabulary", () => {
    ...
    expect(buildCanDoScope(units, "Integration & checkpoint", words)).toBeNull();
  });
});
...
describe("can-do markers", () => {
  ...
  it("ignores real unit ids sharing the list", () => {
  ...
  it("keeps markers distinguishable from unit ids so the ladder ignores them", () => {
    // dailyLoop picks the next unit by id membership; a marker must never
    // collide with a real unit id or it would skip that unit.
