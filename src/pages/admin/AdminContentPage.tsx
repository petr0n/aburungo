...
function CoverageBar({ label, pct, withCount, total }: {
  label: string
  pct: number
  withCount: number
  total: number
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center justify-between">
        ...
        <span className="text-body-sm text-fg-subtle">{withCount} / {total} ({pct}%)</span>
      </div>
...
export function AdminContentPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <section>
        <p className="mb-3 text-body-sm font-medium text-fg-subtle">
          Kanji issues — {data.kanji.length} flagged
        </p>
        {data.kanji.length === 0 ? (
