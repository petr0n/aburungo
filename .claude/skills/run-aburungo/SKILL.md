...
`.env.local` (gitignored) must exist or the app throws at import time:

```
VITE_SUPABASE_URL=
VITE_SUPABASE_PUBLISHABLE_KEY=
VITE_API_URL=
WALKTHROUGH_EMAIL=      # optional; lets pnpm walkthrough sign in
...
in `.claude/skills/run-aburungo/shots/` (gitignored). Output when it passes:
...
| `✗ vite preview never answered` | Something else holds :4188. `lsof -ti:4188 \| xargs kill` |
